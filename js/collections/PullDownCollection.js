/**
 * Created by Bill on 2016/11/1.
 */
define(["jQuery", "underscore", "Backbone",
    "/Bill.io/js/models/PullDownModel.js"
    ], function($, _, Backbone, PullDownModel){

        var PullDownCollection = Backbone.Collection.extend({
            model: PullDownModel,

            url: "/Bill.io/json/scrollLoading.json",

            /*
             This function is used to handle the response received from the server
             before setting the response to the collection.
             */
            parse: function(serverResponse, xhr){
                console.log("parse PullDown collection start");
                if(serverResponse){
                    var content = serverResponse["content"];
                    return content;
                }
                return serverResponse;
            },

            initialize: function(){
                console.log("PullDownCollection initialize...");
            }
        });

        return PullDownCollection;
});
